...
class ResCompany(models.Model):
    ...
    def _get_company_address_field_names(self):
        return (super()._get_company_address_field_names() +
                ['street_number', 'street_number2', 'l10n_mx_locality', 'l10n_mx_locality_id', 'l10n_mx_colony', 'l10n_mx_colony_code', 'district_id', 'city_id'])
    ...
    def _onchange_zip(self):
        ...
        if self.zip and self.country_enforce_districts and (not self.district_id or self.zip != self.district_id.zip_code):
            ...
            self.city_id = districts[0].city_id.id
            self.city = districts[0].city_id.name
            self.state_id = districts[0].city_id.state_id.id
            self.country_id = districts[0].city_id.state_id.country_id.id
