...
class SoltApiMetaFields(models.Model):
    ...
    extended_form_view_id = fields.Many2one('ir.ui.view', 'Vista de formulario extendida', readonly=True,
                                   help="The form view of the model that want to extend")
    ...
    def onchange_model_id(self):
        ...
        if self.model_id:
            ...
            form_view = view_obj.search(
                [('model', '=', self.model_id.model), ('type', '=', 'form'), ('mode', '=', 'primary'),
                 ('inherit_id', '=', False)], limit=1)
            if form_view:
                values['form_view_id'] = form_view.id
            search_view = view_obj.search(
                [('model', '=', self.model_id.model), ('type', '=', 'search'), ('mode', '=', 'primary')], limit=1)
            if search_view:
    ...
    def _get_meta_fields_lines(self):
        ...
        for fdict in fields_to_load:
            fdict.update({
                    ...
                    'modules': 'solt_api_connector, ' + self.model_id.modules
                })
    ...
    def make_form_view(self):
        ...
        view_data = {'name': f'solt_api_connector{self.id}.{self.model}.form.view', 'type': 'form',
                     'model': self.model, 'inherit_id': self.form_view_id.id, 'mode': 'extension',
        ...
        if not view:
            ...
            self.env['ir.model.data']._update_xmlids([{
                'xml_id': f"solt_api_connector.solt_api_connector{self.id}_{self.model.replace('.', '_')}_form_view",
                'record': view,
                'noupdate': True,
            }])
    ...
    def make_search_view(self):
        ...
        if self.search_view_id:
            ...
            if state_field:
                ...
                view_data = {'name': f"{self.model}.{self.search_view_id.id}.api.connector.search.view", 'type': 'search',
                             'model': self.model, 'inherit_id': self.search_view_id.id, 'mode': 'extension',
                             'arch': tostring(data, pretty_print=True)}
                # update or create view
    ...
    def make_tree_view(self):
        ...
        if self.tree_view_id:
            ...
            view_data = {'name': f"{self.model}.{self.tree_view_id.id}.api.connector.list.view", 'type': 'list',
                         ...
                         'arch': tostring(data, pretty_print=True)}
            # update or create view
            xml_id = f"solt_api_connector.{self.model.replace('.', '_')}_{self.tree_view_id.id}_api_connector_list_view"
            view = self.env.ref(xml_id, raise_if_not_found=False)
    ...
    def add_dynamic_fields_to_extended_view(self, model, new_field_names):
        ...
        for mf in new_field_names:
            if mf in fields_attrs:
                continue
                # raise UserError(_('Field %s already exists, the same field can only be in one.' % mf))
