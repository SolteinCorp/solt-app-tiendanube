...
class ConnectorSyncMixin(models.AbstractModel):
    ...
    def clear_sync_flags(self):
        ...
        if records_to_clear:
            _logger.info("Clearing synchronization flags for %s records of the model %s", len(records_to_clear), self._name)
    ...
    def clear_all_sync_flags(self):
        ...
        for model_name in self.env['connector.sync.mixin']._inherit_children:
            try:
                ...
                _logger.error("Error clearing synchronization flags for the model %s: %s", model_name, e)
