...
""", default="{}")
    ...
    def execute_request(self, record=None, params=None, data=None, request_data=None):
        ...
        log_vals = {'connector_id': connector.id, 'endpoint_id': self.id, 'company_id': self.env.company.id,
                    'request_url': url, 'request_method': self.method, 'request_headers': json.dumps(headers),
        ...
        try:
            ...
            log_vals.update({'response_code': status_code, 'response_headers': json.dumps(dict(response.headers)),
                             'response_body': response.text, 'success': 200 <= status_code < 300,
            ...
            end_time = fields.Datetime.now()
            log_vals.update({'response_code': 0, 'response_body': str(e), 'success': False,
                             'duration': (end_time - start_time).total_seconds(), })
            created_log = self.env['solt.api.call.log'].create(log_vals)
            created_log._init_retry_if_needed()
    ...
    def _execute_paginated_request(self, record=None, params=None, data=None):
        ...
        while has_more_pages:
            ...
            try:
                ...
                log_vals.update(
                    {'response_code': response.status_code, 'response_headers': json.dumps(dict(response.headers)),
                     'response_body': response.text, 'success': 200 <= response.status_code < 300,
