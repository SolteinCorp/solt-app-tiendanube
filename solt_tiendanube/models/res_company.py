...
class ResCompany(models.Model):
    ...
    use_sale_multi_stock = fields.Boolean(
        string="Use multi warehouse in Sales Orders",
        readonly=False,
    ...
    def _sync_inventory(self, offset, limit):
        ...
        _logger.info("Starting Cron: Sync Inventory with the parameters [{}|{}]".format(offset, limit))
        ...
        try:
            for company in stores:
                ...
                endpoint = self.env["solt.api.endpoint"].search(
                    [("code", "=", "PRODUCT_VARIANT_STOCK_PRICE_PATCH"), ("connector_id", "=", company.connector_id.id)], limit=1
                )
                if not endpoint:
                    raise UserError(_("Endpoint PRODUCT_VARIANT_STOCK_PRICE_PATCH is not configured!"))
                request_mapping_config = json.loads(endpoint.request_mapping)
                if not request_mapping_config:
                    raise UserError(_("Endpoint PRODUCT_VARIANT_STOCK_PRICE_PATCH is misconfigured!"))

                while True:
                    records_paginated = utils.search_paginated(
                        self.env, model_name, company, domain, limit_max=limit_max, offset=offset_arg, limit=limit
                    )
                    ...
                    if request_data and products:
                        ...
                        if 200 <= response.get("status_code") < 300:
                            ...
                            _logger.error(f"Error updating product stock: {response.get('error_message', '')}")

                    count_updated += len(products)
                    if count_updated < total_products:
                        offset_arg += limit
                        _logger.info("Updated {} records of {}".format(count_updated, total_products))
                    else:
                        _logger.info("Updated {} records of {}".format(count_updated, total_products))
                        break
    ...
    def _sync_variants_cost(self, offset, limit):
        ...
        _logger.info("Starting Cron: Tiendanube sync variants cost with the parameters [{}|{}]".format(offset, limit))
        ...
        try:
            for company in stores:
                ...
                if product_id:
                    ...
                    domain = [("x_external_id", "not in", ["", False]), ("x_exclud_from_sync", "=", False), ("company_id", "=", company.id)]
                limit_max = self.env[model_name].with_company(company).search_count(domain)
                ...
                while True:
                    ...
                    for product in products:
                        ...
                        endpoint = self.env["solt.api.endpoint"].search(
                            [("code", "=", code), ("connector_id", "=", company.connector_id.id)], limit=1
                        )
                        if not endpoint:
                            ...
                            for variant in product.product_variant_ids:
                                if variant.standard_price > 1:
                                    val = {
                                        ...
                                        "values": [
                                            variant.convert_translated_field_to_api_format(av.name)
                        ...
                        _logger.info(response)
                        sync_vals = {"x_state_sync": "yes", "x_exclud_from_sync": False, "x_date_last_sync": fields.Datetime.now()}
                        if response and 200 <= response.get("status_code") < 300:
                            ...
                            _logger.error(f"Error updating product {product.name} cost: {response.get('error_message', '')}")
    ...
    def _get_installed_modules_info(self):
        ...
        try:
            ...
            soltein_modules = self.env["ir.module.module"].sudo().search([("author", "ilike", "Soltein SA de CV"), ("state", "=", "installed")])
            modules_list = []
            ...
            return {"count": len(modules_list), "modules": modules_list}
